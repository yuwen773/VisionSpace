...
// 空间级别选项映射
export const SPACE_LEVEL_OPTIONS: { label: string; value: number }[] = Object.keys(
  SPACE_LEVEL_MAP,
).map((key: string): { label: string; value: number } => {
  const value: number = Number(key) // 将字符串键转换为数字
  return {
